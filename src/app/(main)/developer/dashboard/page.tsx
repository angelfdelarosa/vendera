  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        if (profile) {
          // Load projects and interests
          const [projectsData, interestsData] = await Promise.all([
            developerService.getProjectsByDeveloper(profile.id),
            developerService.getInterestsByDeveloper(profile.id)
          ]);
        ...
        <Card className="max-w-md mx-auto">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-gray-600 mb-4">
              Debes iniciar sesión para acceder al dashboard de desarrollador.
            </p>
      ...
      <Tabs defaultValue="projects" className="w-full">
                      ...
                      <div className="flex-1">
                        <div className="flex flex-col md:flex-row md:items-center gap-2 mb-2">
                          ...
                          <div className="flex gap-2">
                            ...
                            <Badge className={`text-xs ${interestStatusColors[interest.status]}`}>
                        ...
                        {interest.message && (
                          <p className="text-xs md:text-sm text-gray-600 mb-2">{interest.message}</p>
                        )}
                        
                        <div className="flex flex-col md:flex-row md:items-center gap-2 text-xs md:text-sm text-gray-500">
                          <span>Contacto preferido: {interest.contact_preference}</span>
                          <span className="hidden md:inline">•</span>
                          <span>{new Date(interest.created_at).toLocaleDateString('es-ES')}</span>
                        </div>
                      </div>
        ...
        <TabsContent value="analytics" className="mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-base md:text-lg">Rendimiento de Proyectos</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {projects.map((project) => {
                      ...
                      <div key={project.id} className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          <p className="text-xs md:text-sm font-medium text-green-600">
                            {((projectInterests / views) * 100).toFixed(1)}% conversión
                          </p>
                        </div>
                      </div>
                    );
